### Cервис доставки еды Gastro Hub Delivery - Part 1
...
Проблема:
  ...
  Решение
  1) Т.к. в отчет не включается текущий день создадим materialized view, который можно будет обновлять утром следующего дня.
```sql
